...
``` conf
[all]
k3smaster ansible_ssh_host=192.168.4.200
...
On completion, of this operation, you can copy the Kubernetes config for the primary node to your local machine. Run the command `scp root@<MASTER IP>:/etc/rancher/k3s/k3s.yaml ~/.kube/config` and replace `<MASTER IP>` with your primary node's IP. You will need to modify the local `~/.kube/config` file on your computer. Run `nano ~/.kube/config` and change the line that defines `server:` from `127.0.0.1` to the IP of your primary node.

Now get the Node Token that authorizes additional nodes to interact with your primary node. To do this run `ssh node@<MASTER IP> "sudo cat /var/lib/rancher/k3s/server/node-token"` with your primary nodes IP and make note of the output. Now to install k3s on the additional nodes run `ansible k3snodes -e K3S_TOKEN=<TOKEN> -e K3S_URL="https://<MASTER IP>:6443" -e K3S_NODE_NAME={{inventory_hostname}} -a "bash /tmp/k3s" -v`. Replace `<TOKEN>` with the token you got from the output of the previous command, and `<MASTER IP>` with the IP of the primary node. Verify that all the nodes are integrated with the cluster by running `kubectl get nodes`. You should see the following output:
